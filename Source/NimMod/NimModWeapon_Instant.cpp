...
#include "NimMod.h"
#include "NimModWeapon_Instant.h"
...
void ANimModWeapon_Instant::ServerNotifyHit_Implementation(const FHitResult Impact, FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
bool ANimModWeapon_Instant::ServerNotifyMiss_Validate(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	return true;
}

void ANimModWeapon_Instant::ServerNotifyMiss_Implementation(FVector_NetQuantizeNormal ShootDir, int32 RandomSeed, float ReticleSpread)
{
	const FVector Origin = GetMuzzleLocation();
	...
	if (GetNetMode() != NM_DedicatedServer)
	{
		...
		SpawnTrailEffect(EndTrace, ShootDir.Rotation());
	}
}

void ANimModWeapon_Instant::ProcessInstantHit(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
...
void ANimModWeapon_Instant::ProcessInstantHit_Confirmed(const FHitResult& Impact, const FVector& Origin, const FVector& ShootDir, int32 RandomSeed, float ReticleSpread)
{
	// handle damage
	...
	if (GetNetMode() != NM_DedicatedServer)
	{
		...
		SpawnTrailEffect(EndPoint, ShootDir.Rotation());
		SpawnImpactEffects(Impact);
	}
}

bool ANimModWeapon_Instant::ShouldDealDamage(AActor* TestActor) const
{
...
void ANimModWeapon_Instant::DealDamage(const FHitResult& Impact, const FVector& ShootDir)
{
...
void ANimModWeapon_Instant::OnBurstFinished()
{
...
void ANimModWeapon_Instant::OnRep_HitNotify()
{
	SimulateInstantHit(HitNotify.Origin, HitNotify.RandomSeed, HitNotify.ReticleSpread);
}

void ANimModWeapon_Instant::SimulateInstantHit(const FVector& ShotOrigin, int32 RandomSeed, float ReticleSpread)
{
	FRandomStream WeaponRandomStream(RandomSeed);
	...
	{
		SpawnImpactEffects(Impact);
		SpawnTrailEffect(Impact.ImpactPoint, Impact.ImpactPoint.Rotation());
	}
	else
	{
		SpawnTrailEffect(EndTrace, ShootDir.Rotation());
...
void ANimModWeapon_Instant::SpawnImpactEffects(const FHitResult& Impact)
{
...
void ANimModWeapon_Instant::SpawnTrailEffect(const FVector& EndPoint, const FRotator& rotation)
{
	if (TrailFX)
	{
		const FVector Origin = GetMuzzleLocation();

		UParticleSystemComponent* TrailPSC = UGameplayStatics::SpawnEmitterAtLocation(this, TrailFX, Origin, rotation);
		if (TrailPSC)
		{
...
void ANimModWeapon_Instant::GetLifetimeReplicatedProps(TArray< FLifetimeProperty > & OutLifetimeProps) const
{
	Super::GetLifetimeReplicatedProps(OutLifetimeProps);

	DOREPLIFETIME_CONDITION(ANimModWeapon_Instant, HitNotify, COND_SkipOwner);
}
